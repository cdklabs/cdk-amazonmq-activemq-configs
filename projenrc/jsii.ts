...
export class Jsii extends pj.Component {
  ...
  constructor(
    ...
    const jsiiFlags = ['--silence-warnings=reserved-word', '-c ./tsconfig.json'];
    if (compressAssembly) {
      jsiiFlags.push('--compress-assembly');
    }

    project.addFields({ stability: options.stability ?? 'experimental' });
    ...
    new pj.JsonFile(this.project, 'tsconfig.json', {
      obj: {
        compilerOptions: {
          outDir: 'lib',
          rootDir: 'src',
